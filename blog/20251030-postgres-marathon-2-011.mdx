...
|---------|--------|----------|--------|---------|----------|-------|
| `auto` | 8 locks | 8 locks | 52 locks (eval) | 8 or 13 locks* | Every time | *8 if rejects generic (data), 13 if accepts (empty) |
...
The ultimate irony: we use prepared statements to avoid re-planning and reduce lock contention, but with partitioned tables, we need to disable the prepared statement optimization (plan caching) to avoid lock contention.
