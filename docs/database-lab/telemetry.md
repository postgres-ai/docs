...
- For DLE Community Edition (DLE CE, open-source version), the data collected is minimal and non-personally identifiable. Particularly, the DLE instance ID is randomly generated and contains no identifying information. For DLE Standard Edition and DLE Enterprise Edition (DLE SE and DLE EE, respectively – paid versions installed from cloud Marketplaces or from the Postgres.ai Console), the DLE instance ID is stored in the PostgresAI SaaS database in the context of the customer's organization and, upon request, may be used for support purposes.
...
- For DLE SE, telemetry cannot be disabled because it is used for linking to the customer's organization in PostgresAI Console, for billing and support purposes.
...
If the change is done when DLE is running, follow the [DLE reconfiguration guide](/docs/dblab-howtos/administration/engine-manage#reconfigure-dblab-engine) to apply the change without restart.
