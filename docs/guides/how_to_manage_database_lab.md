...
Define config file `~/.dblab/configs/config.yml` according to [example](https://gitlab.com/postgres-ai/database-lab/-/blob/master/configs/config.sample.yml) and run the following command:
...
Update the configuration file `~/.dblab/configs/config.yml`.
...
Stop and remove the container using `sudo docker stop dblab_server` and `sudo docker rm dblab_server` After that, [launch](./how-to-manage-database-lab#start-database-lab-instance) a new container.
