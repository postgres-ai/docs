...
### Options
Copy the contents of configuration example [`config.example.physical_walg.yml`](https://gitlab.com/postgres-ai/database-lab/-/blob/v2.1/configs/config.example.physical_walg.yml) from the Database Lab repository to `~/.dblab/server.yml` and update the following options:
