...
In many workloads, `autovacuum` settings — especially the defaults — need to be tuned to keep up with write traffic.
...
👉 If the number of active workers regularly hits `autovacuum_max_workers`, that is a strong signal to raise the
limit (which requires a restart) and/or let workers run faster by
