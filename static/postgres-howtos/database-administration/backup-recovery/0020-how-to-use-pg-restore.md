...
slug: how-to-use-pg-restore
sidebar_label: use pg_restore
description: Today – a few tips on using `pgrestore` to restore databases (or only parts of them) from dumps.
keywords: ["postgresql","pg_restore","intermediate"]
tags: ["intermediate", "backup", "restore", "pg_restore", "parallelization", "recovery"]
...
[pg_dump howto](/docs/postgres-howtos/database-administration/backup-recovery/how-to-speed-up-pg-dump).
...
- **Database Size**: Monitor the restored database size (e.g., with `select pg_database_size('dbname')`). Note that the size might differ from the original due to factors like bloat.
- **System Monitoring**: Utilize system monitoring tools to observe resource usage during the restore process.
...
- **Continue on Error**: Without `--exit-on-error`, only failed statements and their dependent objects must be restored afterward. Again, use a modified TOC list (`pg_restore -l` and `-L`) to selectively restore these entries.

- **Object Dependencies**: Always review the TOC list carefully to understand dependencies between objects and handle them appropriately.


## Permissions, ownership, etc.

`pg_dump` does not include roles and permissions. Use `pg_dumpall --roles-only` to back them up and restore with `psql -f roles.sql` before the main dump to ensure correct ownership and privileges.
