---
title: How to deal with long-running transactions (OLTP)
slug: how-to-deal-with-long-running-transactions-oltp
sidebar_label: deal with long-running transactions (OLTP)
description: >-
  Identify and handle long-running transactions that can impact OLTP system
  performance
keywords:
  - postgresql
  - deal
...
1) A cronjob (or `pg_cron` or `pg_timetable`) record to run a "terminator" query that detects all transactions lasting
   ...
   order by clock_timestamp() - xact_start desc nulls last;
   ```
